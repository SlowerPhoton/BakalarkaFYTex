...
\NazevUstavu{Název ústavu}{Department of Physical Electronics}

\RokOdevzdaniPrace{2022}

\AkademickyRok{2021/2022}

\Autor{Roman Solař}{Roman Solař}

\NazevPrace{Stochastický přístup k řešení plazmové kinetiky}{Stochastický přístup k řešení plazmové kinetiky}{Stochastic approach to plasma kinetics}
...
\VedouciPraceSTituly{Mgr. Zdeněk Bonaventura, Ph.D.} %% Neni potreba pro rigorozni prace
...
\KlicovaSlova{Monte Carlo, equilibrium, plazma, stochastická simulace }{Monte Carlo, equilibrium, plasma, stochastic simulation}

\Abstrakty%
{V této bakalářské práci se zabýváme stochastickou metodou pro simulaci koncentrace částic v plazmatu. Zjištujeme, jaké jsou její teoretické hranice a jak se vypořádat zejména s řádově rozdílnými počatečními koncentracemi. Výsledky simulací ověřujeme s pomocí numerických výpočtů příslušných diferenciálních rovnic. Potřebné reakční rychlosti získáváme z databází experimentálně naměřených dat.}%
{In this thesis, we explore the stochastic approach to particle concentration simulation in plasma. The main focus is put into investigating the theoretical limits of this method, especially when it comes to concentrations of very different orders of magnitude. The simulation outcomes are then compared to the results obtained from numerical solutions to the corresponding differential equations. The necessary reaction rates are taken from databases of experimentally obtained data.}

\TextPodekovani%
{Především bych chtěl poděkovat svému vedoucímu práce Mgr. Zdeňkovi Bonaventurovi, Ph.D., a to za nadstandardní pomoc při řešení problémů. Vždy, když to bylo potřeba, si na mě udělal čas.

Kromě toho děkuji své rodině a přítelkyni, kteří mě při psaní podporovali.
}
...
\DatumProhlaseni{23. května 2022}
...
\HlavickaKapitoly
\vloz{text_prace/simple_mc}
\cleardoublepage

\vloz{text_prace/advanced_mc}
\cleardoublepage

\vloz{text_prace/2reaction}
\cleardoublepage

\vloz{text_prace/micro_cathode}
\cleardoublepage

\vloz{text_prace/implementation}
\cleardoublepage

\vloz{text_prace/conclusion}
\cleardoublepage
